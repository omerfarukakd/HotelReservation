...
class ModernHotelApp(ctk.CTk):
    ...
    def _build_layout(self) -> None:
        ...
        ctk.CTkLabel(brand, text="Rezervasyon & Yönetim", text_color=self.colors["muted"],
                     font=("Segoe UI", 13)).pack(anchor="w")
        
        # Subtle separator
        ctk.CTkFrame(brand, height=1, fg_color=self.colors["line"]).pack(fill="x", pady=(15, 5))
        
        ctk.CTkLabel(brand, text="Ünlü Kodu: STAR2026", text_color=self.colors["text"],
        ...
        nav_btn("book", "Rezervasyon", "🧾")
        nav_btn("reservations", "Kayıtlar", "📚")
        nav_btn("preview", "Otel Önizleme", "🏢")
        
        # User requested specific left alignment for this button
        nav_btn("maintenance", "Tadilat", "🛠️", anchor_override="w")
        
        nav_btn("reports", "Raporlar", "📈")
        nav_btn("help", "DS/Algo", "🧠")
        ...
        self.theme_switch = ctk.CTkSwitch(
            self.topbar, text="Karanlık Mod", variable=self.theme_switch_var, onvalue="dark", offvalue="light",
    ...
    def _page_book(self, master) -> ctk.CTkFrame:
        ...
        form_card = self._card(left, "Rezervasyon Oluştur", "Tarih seçimi takvimlidir • 6 ay sınırı uygulanır")
        ...
        ctk.CTkLabel(leftp, text="Yetişkin", text_color=self.colors["muted"],
        ...
        ctk.CTkLabel(rightp, text="Çocuk", text_color=self.colors["muted"],
                     font=("Segoe UI", 12, "bold")).pack(side="left", padx=(0, 8))
        ...
        self._celeb_label = ctk.CTkLabel(grid_form, text="Ünlü Kodu", text_color=self.colors["muted"], font=("Segoe UI", 12, "bold"))

        field(0, "Misafir Adı", e_guest)
        field(1, "Oda Tipi", cb_type)
        field(2, "Giriş Tarihi", de_ci)
        field(3, "Gece Sayısı", sp_n)
        field(4, "Yetişkin / Çocuk", ppl)
        ...
        # results card (left)
        res_card = self._card(left, "Uygun Odalar", "En ucuzdan listelenir • Oda adı daha okunaklı")
        ...
        srv_card = self._card(right, "Ekstra Hizmetler", "Seçimler fiyata yansır (per-night / per-person vb.)")
        ...
        for sid, srv in self.system.services.items():
            ...
            meta = f"{srv.pricing.get('mode')} • {srv.pricing.get('price')} TL"
            ctk.CTkLabel(line, text=meta, text_color=self.colors["muted"],
                      font=("Segoe UI", 11)).pack(side="right")

        # --- Right: price & occupancy
        price_card = self._card(right, "Fiyat & Doluluk", "Dinamik fiyat: zaman + talep + kişi + hizmetler")
        ...
        ctk.CTkLabel(pc, text="30 Gün Doluluk (yaklaşık)",
        ...
        ctk.CTkLabel(pc, text="İpucu: Doluluk arttıkça talep çarpanı fiyatı yükseltir.",
    ...
    def _page_reservations(self, master) -> ctk.CTkFrame:
        ...
        top = self._card(fr, "Rezervasyon Kayıtları", "Filtrele, incele, iptal et • İptal sonrası bekleme listesi denenir")
        ...
        ctk.CTkButton(act, text="Seçileni İptal Et", fg_color=self.colors["danger"], hover_color=self.colors["danger"], command=self.on_cancel_selected).pack(side="left")
        return fr
    ...
    def _page_preview(self, master) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(head, text="Otel Önizleme", font=("Segoe UI", 20, "bold")).pack(side="left")
        ...
        ctk.CTkButton(head, text="Görüntüle", width=100, command=self._draw_preview_map).pack(side="left", padx=15)
    ...
    def _page_maintenance(self, master) -> ctk.CTkFrame:
        ...
        card = self._card(fr, "Tadilat / Kullanım Dışı", "Kat bazlı veya oda bazlı kapama yapabilirsin")
        ...
        lbl_scope = ctk.CTkLabel(form_grid, text="Tür", text_color=self.colors["muted"],
        ...
        # Dates
        put(3,
            ctk.CTkLabel(form_grid, text="Başlangıç", text_color=self.colors["muted"], font=("Segoe UI", 12, "bold")),
            DateEntry(form_grid, self.m_start, min_date=horizon_min, max_date=horizon_max),
        )
        put(4,
            ctk.CTkLabel(form_grid, text="Bitiş", text_color=self.colors["muted"], font=("Segoe UI", 12, "bold")),
        ...
        act = ctk.CTkFrame(fr, fg_color="transparent")
        act.pack(fill="x", pady=(12, 0))
        ctk.CTkButton(act, text="Seçili Tadilatı İptal Et", fg_color=self.colors["danger"], hover_color=self.colors["danger"], command=self.on_remove_maintenance).pack(side="left")
        return fr


    def _page_reports(self, master) -> ctk.CTkFrame:
        fr = ctk.CTkFrame(master, fg_color="transparent")
        card = self._card(fr, "CSV Raporları", "occupancy_daily.csv • revenue_monthly.csv")
        card.pack(fill="both", expand=True)
        body = card.body  # type: ignore

        ctk.CTkButton(body, text="Doluluk CSV Dışa Aktar", command=self.on_export_occupancy).pack(anchor="w", pady=6)
        ctk.CTkButton(body, text="Gelir CSV Dışa Aktar", command=self.on_export_revenue).pack(anchor="w", pady=6)

        ctk.CTkFrame(body, fg_color=self.colors["line"], height=2).pack(fill="x", pady=15)

        self.summary_var = ctk.StringVar(value="—")
        ctk.CTkLabel(body, text="Hızlı Özet", font=("Segoe UI", 16, "bold")).pack(anchor="w", pady=(0, 6))
    ...
    def _page_help(self, master) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(head, text="Algoritma & Veri Yapıları", font=("Segoe UI", 20, "bold")).pack(side="left")
        ctk.CTkLabel(head, text="Proje Sunum Detayları", text_color=self.colors["muted"], 
...
# ---------- Theme (Dark/Light) ----------
    ...
    def refresh_price_preview(self) -> None:
        try:
            ...
            if not avail:
                ...
                self.price_preview.set(
                    f"{fmt_date(ci)} → {fmt_date(co)}  •  {nights} gece\n"
                    f"Tahmini Toplam: {price} TL\n\n"
                    f"Zaman çarpanı: {tm:.2f}\n"
                    f"Talep çarpanı: {dm:.2f}\n"
                    f"Kişi: {adults} yetişkin + {children} çocuk"
                )
    ...
    def _draw_availability_bars(self, rtype: RoomType) -> None:
        ...
        # title
        c.create_text(pad, pad, anchor="nw", text=f"{rtype} • 30 Gün", fill=self.colors["muted"], font=("Segoe UI", 9, "bold"))
