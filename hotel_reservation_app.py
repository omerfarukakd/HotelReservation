...
# -*- coding: utf-8 -*-
"""
Hotel Reservation System (6-month horizon) — DS/Algo heavy project with GUI
...
class HotelSystem:
    ...
    def try_book(self, guest_name: str, room_type: RoomType, ci: date, nights: int,
        ...
        if not self._is_within_horizon(ci, co):
            return False, "Tarih aralığı 6 ay sınırı dışında veya geçersiz.", None

        if adults <= 0 or (adults + children) <= 0:
            return False, "En az 1 yetişkin olmalı.", None
        ...
        if room_type == "CELEB_FLOOR":
            ...
            if not celeb_ok:
                return False, "Ünlü katı için özel kod gerekli (demo kod: STAR2026).", None
        ...
        if adults + children > max_cap:
            return False, f"Kişi sayısı kapasiteyi aşıyor (max {max_cap}).", None
        ...
        if not available:
            ...
            return False, "Uygun oda yok. Bekleme listesine eklendi; alternatif öneri alabilirsiniz.", None

        # pick best (cheapest computed) — use heapsort to rank
        ...
        return True, f"Rezervasyon oluşturuldu: {res.room_id} / Toplam: {res.total_price} TL", res
    ...
    def cancel(self, res_id: str) -> Tuple[bool, str]:
        ...
        if days_to_ci <= 1:
            return False, "Giriş tarihi yarın/bugün olan rezervasyonlar iptal edilemez."
    ...
    def add_floor_maintenance(self, floor: int, start: date, end: date) -> Tuple[bool, str]:
        ...
        if start < today():
            return False, "Geçmiş tarihe tadilat eklenemez."
        if end > today() + timedelta(days=HORIZON_DAYS):
            return False, "6 ay sınırı dışında."
        ...
        return True, "Tadilat aralığı eklendi."
    ...
    def suggest_knapsack_package(self, budget: int, nights: int, people: int) -> List[Tuple[str, int]]:
        """
        DP (0/1 Knapsack) kullanarak verilen bütçeyi (budget) taşmayacak şekilde
        fiyatı en yüksek (dolayısıyla en kapsamlı) hizmet paketini oluşturur.
        DS&A: Dynamic Programming implementation.
        """
        # 1. Kullanılabilir hizmetleri ve GERÇEK fiyatlarını listele
        items = [] 
        for srv in self.services.values():
            try:
                # Fiyat hesaplama mantığı (Service modeline uygun olmalı)
                # self._services_price metodunu taklit ediyoruz veya direkt çağırabiliriz 
                # ama tek tek hesaplamak için basit mantık kuralım:
                ...
                # fixed ise base_price kalır

                # Mantıksız veya bütçeyi tek başına aşanları ele
    ...
    def suggest_holiday_packages(self, budget: int, start_date: date, nights: int, adults: int, children: int) -> List[Dict]:
        """
        Bütçeye uygun komple tatil paketleri (Oda + Hizmetler) önerir.
        Geriye paket listesi döner. Her paket:
        {
            "room": <Room Object>,
            "services": [(Name, Price), ...],
            "total_price": int,
        ...
        # En pahalı (bütçeyi en çok dolduran) seçenekten ucuza doğru sırala
...
class DatePickerPopup(ctk.CTkToplevel):
    ...
    def _build(self) -> None:
        ...
        btn_prev = tk.Button(header, text="◀", command=self._prev_month,
        ...
        btn_next = tk.Button(header, text="▶", command=self._next_month,
                             bg=self.colors["card"], fg=self.colors["text"], bd=0, padx=10, pady=6,
                             activebackground=self.colors["line"], activeforeground=self.colors["text"], font=("Segoe UI", 12))
        ...
        days_str = ["Pzt", "Sal", "Çar", "Per", "Cum", "Cmt", "Paz"]
...
class DateEntry(ctk.CTkFrame):
    ...
    def __init__(self, master, variable: tk.StringVar, *, min_date: Optional[date] = None, max_date: Optional[date] = None):
        ...
        self.btn = ctk.CTkButton(self, text="📅", width=30, command=self.open)
...
class BudgetWizardDialog(ctk.CTkToplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Bütçe Sihirbazı - Tatil Paketi")
...
class PackageResultDialog(ctk.CTkToplevel):
    def __init__(self, master, packages, on_reserve_callback):
        ...
        ctk.CTkLabel(head, text=f"Bütçenize Uygun {len(packages)} Seçenek", font=("Segoe UI", 14, "bold")).pack(side="left")
    ...
    def _build_card(self, idx, pkg):
        ...
        ctk.CTkLabel(left, text=f"Oda Fiyatı: {room_price} TL", text_color="#AAAAAA", anchor="w").pack(fill="x")
        ...
        ctk.CTkButton(right, text="Seç & Rezerve Et", width=120,
...
TRANSLATIONS = {
    "TR": {
        "app_title": "Otel Rezervasyon",
        "brand_subtitle": "Rezervasyon & Yönetim",
        "celeb_code_hint": "Ünlü Kodu: STAR2026",
        "nav_book": "Rezervasyon",
        "nav_reservations": "Kayıtlar",
        "nav_preview": "Otel Önizleme",
        "nav_maintenance": "Tadilat",
        ...
        "ready": "Hazır.",
        "page": "Sayfa",
        "book_title": "Rezervasyon Oluştur",
        "book_subtitle": "Tarih seçimi takvimlidir • 6 ay sınırı uygulanır",
        ...
        "adults": "Yetişkin",
        "children": "Çocuk",
        "celeb_code": "Ünlü Kodu",
        "search_btn": "Uygun Oda Ara",
        "book_btn": "Rezervasyon Yap",
        "suggest_btn": "Alternatif Öner (A*)",
        "wizard_btn": "Bütçe Sihirbazı (DP)",
        "results_title": "Uygun Odalar",
        "results_subtitle": "En ucuzdan listelenir • Oda adı daha okunaklı",
        "services_title": "Ekstra Hizmetler",
        "services_subtitle": "Seçimler fiyata yansır (per-night / per-person vb.)",
        "price_title": "Fiyat & Doluluk",
        "price_subtitle": "Dinamik fiyat: zaman + talep + kişi + hizmetler",
        "occupancy_title": "30 Gün Doluluk (yaklaşık)",
        "occupancy_hint": "İpucu: Doluluk arttıkça talep çarpanı fiyatı yükseltir.",
        "res_title": "Rezervasyon Kayıtları",
        "res_subtitle": "Filtrele, incele, iptal et • İptal sonrası bekleme listesi denenir",
        "active": "Aktif",
        "archive": "Arşiv",
        "search_lbl": "Ara:",
        "apply": "Uygula",
        "refresh": "Yenile",
        "cancel_selected": "Seçileni İptal Et",
        "preview_title": "Otel Önizleme",
        "preview_date": "Tarih:",
        "view_btn": "Görüntüle",
        "legend_avail": "Uygun",
        "legend_full": "Dolu",
        "legend_maint": "Bakım",
        "maint_title": "Tadilat / Kullanım Dışı",
        "maint_subtitle": "Kat bazlı veya oda bazlı kapama yapabilirsin",
        "type": "Tür",
        "floor": "Kat",
        "room": "Oda",
        "start": "Başlangıç",
        "end": "Bitiş",
        "add_maint": "Tadilat Ekle",
        "remove_maint": "Seçili Tadilatı İptal Et",
        "reports_title": "CSV Raporları",
        "reports_subtitle": "occupancy_daily.csv • revenue_monthly.csv",
        "export_occ": "Doluluk CSV Dışa Aktar",
        "export_rev": "Gelir CSV Dışa Aktar",
        "quick_summary": "Hızlı Özet",
        "help_title": "Algoritma & Veri Yapıları",
        "help_subtitle": "Proje Sunum Detayları",
        "undo": "Undo",
        "redo": "Redo",
        "undo": "Undo",
        "redo": "Redo",
        "srv_breakfast": "Kahvaltı Paketi",
        "srv_spa": "Spa Günü",
        "srv_transfer": "Havalimanı Transfer",
        "srv_late": "Geç Çıkış",
        "srv_extra_bed": "Ek Yatak",
        "srv_security": "Özel Güvenlik (Ünlü Kat)",
        "mode_flat": "sabit",
        "mode_per_person": "kişi başı",
        "mode_per_night": "gecelik",
        "mode_per_person_per_night": "kişi/gece",
        "val_currency": "TL",
        "no_room_msg": "Uygun oda yok.\n• Bekleme listesine eklenir\n• A* ile alternatif öner al",
        "preview_line1": "{start} → {end}  •  {nights} gece",
        "preview_line2": "Tahmini Toplam: {price} {currency}",
        "preview_line3": "Zaman çarpanı: {tm:.2f}",
        "preview_line4": "Talep çarpanı: {dm:.2f}",
        "preview_line5": "Kişi: {adults} yetişkin + {children} çocuk",
        "chart_title": "{rtype} • 30 Gün",
    },
    "EN": {
        "app_title": "Hotel Reservation",
        "brand_subtitle": "Reservation & Management",
        "celeb_code_hint": "Celeb Code: STAR2026",
        "nav_book": "Booking",
        "nav_reservations": "Records",
        "nav_preview": "Hotel Preview",
        ...
        "ready": "Ready.",
        "page": "Page",
        "book_title": "Create Reservation",
        "book_subtitle": "Date selection is calendared • 6 month limit applies",
        ...
        "adults": "Adults",
        "children": "Children",
        "celeb_code": "Celeb Code",
        "search_btn": "Search Rooms",
        "book_btn": "Make Reservation",
        "suggest_btn": "Suggest Alt (A*)",
        "wizard_btn": "Budget Wizard (DP)",
        "results_title": "Available Rooms",
        "results_subtitle": "Listed from cheapest • Room name legible",
        "services_title": "Extra Services",
        "services_subtitle": "Choices affect price (per-night / per-person etc.)",
        "price_title": "Price & Occupancy",
        "price_subtitle": "Dynamic price: time + demand + people + services",
        "occupancy_title": "30 Day Occupancy (approx)",
        "occupancy_hint": "Tip: As occupancy increases, demand multiplier raises price.",
        "res_title": "Reservation Records",
        "res_subtitle": "Filter, inspect, cancel • Waitlist tried after cancel",
        "active": "Active",
        "archive": "Archive",
        "search_lbl": "Search:",
        "apply": "Apply",
        ...
        "preview_title": "Hotel Preview",
        "preview_date": "Date:",
        "view_btn": "View",
        "legend_avail": "Avail",
        "legend_full": "Full",
        "legend_maint": "Maint",
        "maint_title": "Maintenance / Out of Order",
        "maint_subtitle": "Close by floor or by room",
        "type": "Type",
        "floor": "Floor",
        "room": "Room",
        "start": "Start",
        "end": "End",
        "add_maint": "Add Maintenance",
        "remove_maint": "Cancel Selected Maint",
        "reports_title": "CSV Reports",
        "reports_subtitle": "occupancy_daily.csv • revenue_monthly.csv",
        "export_occ": "Export Occupancy CSV",
        "export_rev": "Export Revenue CSV",
        "quick_summary": "Quick Summary",
        "help_title": "Algorithms & Data Structures",
        "help_subtitle": "Project Presentation Details",
        "undo": "Undo",
        "redo": "Redo",
        "undo": "Undo",
        "redo": "Redo",
        "srv_breakfast": "Breakfast Package",
        "srv_spa": "Spa Day",
        "srv_transfer": "Airport Transfer",
        "srv_late": "Late Checkout",
        "srv_extra_bed": "Extra Bed",
        "srv_security": "Private Security (VIP Floor)",
        "mode_flat": "flat",
        "mode_per_person": "per person",
        "mode_per_night": "per night",
        "mode_per_person_per_night": "person/night",
        "val_currency": "TL",
        "no_room_msg": "No room available.\n• Added to waitlist\n• Use A* for suggestions",
        "preview_line1": "{start} → {end}  •  {nights} nights",
        "preview_line2": "Est. Total: {price} {currency}",
        "preview_line3": "Time Multiplier: {tm:.2f}",
        "preview_line4": "Demand Multiplier: {dm:.2f}",
        "preview_line5": "People: {adults} adults + {children} children",
        "chart_title": "{rtype} • 30 Days",
    }
}
...
class ModernHotelApp(ctk.CTk):
    def __init__(self, system: HotelSystem):
        ...
        self.lang = "TR"  # Default Language

        ctk.set_appearance_mode("Dark")
        ctk.set_default_color_theme("blue")

        self.title("Otel Rezervasyon — Modern GUI (CustomTkinter)")
        ...
        self.reservations_view_mode = ctk.StringVar(value="Aktif")
    ...
    def toggle_language(self, new_lang: str) -> None:
    ...
    def _build_layout(self) -> None:
        ...
        ctk.CTkLabel(brand, text=self._t("brand_subtitle"), text_color=self.colors["muted"],
                     font=("Segoe UI", 13)).pack(anchor="w")
        
        # Subtle separator
        ctk.CTkFrame(brand, height=1, fg_color=self.colors["line"]).pack(fill="x", pady=(15, 5))
        
        ctk.CTkLabel(brand, text=self._t("celeb_code_hint"), text_color=self.colors["text"],
        ...
        def nav_btn(key: str, text: str, icon: str):
            b = ctk.CTkButton(nav, text=f"  {icon}  {text}", anchor="w",
        ...
        nav_btn("book", self._t("nav_book"), "🧾")
        nav_btn("reservations", self._t("nav_reservations"), "📚")
        nav_btn("preview", self._t("nav_preview"), "🏢")
        nav_btn("maintenance", self._t("nav_maintenance"), "🛠️")
        nav_btn("reports", self._t("nav_reports"), "📈")
        nav_btn("help", self._t("nav_help"), "🧠")

        # Sidebar footer actions
        foot = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        foot.pack(side="bottom", fill="x", padx=10, pady=20)

        ctk.CTkButton(foot, text=self._t("undo"), fg_color=self.colors["card"], text_color=self.colors["text"],
                      hover_color=self.colors["line"], border_width=1, border_color=self.colors["line"],
                      command=self.on_undo).pack(fill="x", pady=4)
        ctk.CTkButton(foot, text=self._t("redo"), fg_color=self.colors["card"], text_color=self.colors["text"],
                      hover_color=self.colors["line"], border_width=1, border_color=self.colors["line"],
        ...
        ctk.CTkLabel(self.topbar, text=self._t("app_title"), font=("Segoe UI", 24, "bold")).pack(side="left")
        ...
        self.theme_switch = ctk.CTkSwitch(
            self.topbar, text=self._t("dark_mode") if self.theme == "dark" else self._t("light_mode"), 
        ...
        self.lang_switch = ctk.CTkSegmentedButton(
            self.topbar, values=["TR", "EN"], command=self.toggle_language, width=80
        )
        self.lang_switch.set(self.lang)
        self.lang_switch.pack(side="right", padx=(20, 0))

        self.kpi_var = ctk.StringVar(value="—")
        ...
        self.show_page("book")
        self.title(self._t("app_title")) # Update window title
    ...
    def _page_book(self, master) -> ctk.CTkFrame:
        ...
        form_card = self._card(left, self._t("book_title"), self._t("book_subtitle"))
        ...
        ctk.CTkLabel(leftp, text=self._t("adults"), text_color=self.colors["muted"],
        ...
        ctk.CTkLabel(rightp, text=self._t("children"), text_color=self.colors["muted"],
                     font=("Segoe UI", 12, "bold")).pack(side="left", padx=(0, 8))
        ...
        self._celeb_label = ctk.CTkLabel(grid_form, text=self._t("celeb_code"), text_color=self.colors["muted"], font=("Segoe UI", 12, "bold"))

        field(0, self._t("guest_name"), e_guest)
        field(1, self._t("room_type"), cb_type)
        field(2, self._t("checkin_date"), de_ci)
        field(3, self._t("nights"), sp_n)
        field(4, self._t("adults") + " / " + self._t("children"), ppl)
        ...
        ctk.CTkButton(act, text=self._t("search_btn"), command=self.on_search).pack(side="left", padx=(0, 10))
        ctk.CTkButton(act, text=self._t("book_btn"), command=self.on_book).pack(side="left", padx=(0, 10))
        ctk.CTkButton(act, text=self._t("suggest_btn"), fg_color="transparent", border_width=1, 
                      text_color=self.colors["text"], command=self.on_suggest).pack(side="left")
        ctk.CTkButton(act, text=self._t("wizard_btn"), fg_color="#7C3AED", 
                      hover_color="#6D28D9", width=140,
                      command=self.on_knapsack_suggest).pack(side="left", padx=(10, 0))

        # results card (left)
        res_card = self._card(left, self._t("results_title"), self._t("results_subtitle"))
        ...
        srv_card = self._card(right, self._t("services_title"), self._t("services_subtitle"))
        ...
        for sid, srv in self.system.services.items():
            ...
            # Determine display name key from service_id or name
            # Exact mapping from services.json names
            name_map = {
                "Kahvaltı Paketi": "srv_breakfast",
                "Spa Günü": "srv_spa",
                "Havalimanı Transfer": "srv_transfer",
                "Geç Çıkış": "srv_late",
                "Ek Yatak": "srv_extra_bed",
                "Özel Güvenlik (Ünlü Kat)": "srv_security"
            }
            display_name = self._t(name_map.get(srv.name, srv.name)) # fallback to raw name if no key
            ...
            p_mode = srv.pricing.get('mode', '')
            # map mode to key: flat -> mode_flat, per_night -> mode_per_night, etc
            mode_key = f"mode_{p_mode}"
            display_mode = self._t(mode_key)
            
            meta = f"{display_mode} • {srv.pricing.get('price')} {self._t('val_currency')}"
            ctk.CTkLabel(line, text=meta, text_color=self.colors["muted"],
                      font=("Segoe UI", 11)).pack(side="right")

        # --- Right: price & occupancy
        price_card = self._card(right, self._t("price_title"), self._t("price_subtitle"))
        price_card.pack(fill="both", expand=True, pady=(20, 0))
        pc = price_card.body  # type: ignore

        self.price_preview = ctk.StringVar(value="—")
        ...
        ctk.CTkLabel(pc, text=self._t("occupancy_title"),
        ...
        ctk.CTkLabel(pc, text=self._t("occupancy_hint"),
    ...
    def _page_reservations(self, master) -> ctk.CTkFrame:
        ...
        top = self._card(fr, self._t("res_title"), self._t("res_subtitle"))
        top.pack(fill="both", expand=True)

        body = top.body  # type: ignore

        # View Mode Toggle (Active / Archive)
        # "Kayıtlar kısmının içine Arşiv sectionu ekle"
        mode_row = ctk.CTkFrame(body, fg_color="transparent")
        mode_row.pack(fill="x", pady=(0, 10))
        
        # Determine current mode or default
        current_mode = self.reservations_view_mode.get()
        # If switching lang, current_mode might be old lang string. Reset to Active if not matching?
        # Better: map internal state? For simplicity, just set to translated 'Active' if invalid.
        
        valid_modes = [self._t("active"), self._t("archive")]
        if current_mode not in valid_modes:
             current_mode = self._t("active")
             self.reservations_view_mode.set(current_mode)
        
        seg = ctk.CTkSegmentedButton(mode_row, values=valid_modes, 
                                     variable=self.reservations_view_mode,
                                     command=lambda v: self.refresh_reservations())
        seg.pack(side="left")
        seg.set(current_mode) # Ensure visual sync

        # filter row
        frow = ctk.CTkFrame(body, fg_color="transparent")
        frow.pack(fill="x", pady=(0, 15))
        ctk.CTkLabel(frow, text=self._t("search_lbl"), text_color=self.colors["muted"], 
        ...
        ctk.CTkButton(frow, text=self._t("apply"), width=80, command=self.refresh_reservations).pack(side="left", padx=(0, 8))
        ctk.CTkButton(frow, text=self._t("refresh"), width=80, command=self.refresh_all).pack(side="left")
        ...
        ctk.CTkButton(act, text=self._t("cancel_selected"), fg_color=self.colors["danger"], hover_color=self.colors["danger"], command=self.on_cancel_selected).pack(side="left")
        return fr
    ...
    def _page_preview(self, master) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(head, text=self._t("preview_title"), font=("Segoe UI", 20, "bold")).pack(side="left")
        ctk.CTkLabel(head, text=self._t("preview_date"), font=("Segoe UI", 12, "bold"), text_color=self.colors["muted"]).pack(side="left", padx=(30, 10))
        
        self.preview_date_var = ctk.StringVar(value=fmt_date(today()))
        DateEntry(head, self.preview_date_var).pack(side="left")
        
        ctk.CTkButton(head, text=self._t("view_btn"), width=100, command=self._draw_preview_map).pack(side="left", padx=15)
        ...
        _dot("#2CC985", self._t("legend_avail"))
        _dot("#E11D48", self._t("legend_full"))
        _dot("#94A3B8", self._t("legend_maint"))
    ...
    # ---------------------------------------------------------
    # DEĞİŞTİR: ModernHotelApp Class'ındaki _draw_preview_map metodu
    # ---------------------------------------------------------
    def _draw_preview_map(self) -> None:
        ...
        for fl in floors:
            ...
            for room in rooms_on_floor:
                ...
                # --- EKONOMİ GÖRSELLEŞTİRME (HEATMAP) ---
                # Odanın talep çarpanını (Demand Multiplier) al
                dm = self.system._demand_multiplier(room.type, target_date, next_day)
                
                if in_maint:
                    color = "#475569" # Gri (Bakım)
                    outline = "#1E293B"
                    textc = "#CBD5E1"
                    price_text = "BAKIM"
                elif in_res:
                    color = "#E11D48" # Kırmızı (Dolu)
                    outline = "#881337"
                    textc = "#FFE4E6"
                    price_text = "DOLU"
                else:
                    # Renk Skalası: Yeşil (Ucuz) -> Turuncu -> Kırmızı (Pahalı)
                    # dm genellikle 1.0 (Normal) ile 1.6 (Çok Yoğun) arasında değişir.
                    ratio = (dm - 1.0) / 0.6 
                    ratio = max(0.0, min(1.0, ratio))
                    
                    # RGB İnterpolasyon (Yeşil #10B981 -> Kırmızıya yakın bir ton)
                    r = int(16 + (220-16) * ratio)
                    g = int(185 - (50-185) * ratio) # Green decreases
                    b = int(129 - (80-129) * ratio)
                    
                    # Daha canlı renkler için basit mantık:
                    # Düşük talep: Yeşil, Yüksek talep: Turuncu/Kiremit
                    if ratio < 0.3: color = "#10B981" # Yeşil
    ...
    def _page_maintenance(self, master) -> ctk.CTkFrame:
        ...
        card = self._card(fr, self._t("maint_title"), self._t("maint_subtitle"))
        ...
        room_values = [f"{room.room_id} — {room.display_name}" for room in self.system.rooms.values()]
        ...
        lbl_scope = ctk.CTkLabel(form_grid, text=self._t("type"), text_color=self.colors["muted"],
                             font=("Segoe UI", 12, "bold"))
        cb_scope = ctk.CTkComboBox(form_grid, variable=self.m_scope, values=["floor", "room"], state="readonly")
        put(0, lbl_scope, cb_scope)

        # Floor row
        self._mw_floor_label = ctk.CTkLabel(form_grid, text=self._t("floor"), text_color=self.colors["muted"],
                                        font=("Segoe UI", 12, "bold"))
        ...
        # Room row
        self._mw_room_label = ctk.CTkLabel(form_grid, text=self._t("room"), text_color=self.colors["muted"],
                                       font=("Segoe UI", 12, "bold"))
        ...
        # Dates
        put(3,
            ctk.CTkLabel(form_grid, text=self._t("start"), text_color=self.colors["muted"], font=("Segoe UI", 12, "bold")),
            DateEntry(form_grid, self.m_start, min_date=horizon_min, max_date=horizon_max),
        )
        put(4,
            ctk.CTkLabel(form_grid, text=self._t("end"), text_color=self.colors["muted"], font=("Segoe UI", 12, "bold")),
            DateEntry(form_grid, self.m_end, min_date=horizon_min, max_date=horizon_max),
        )

        btn_row = ctk.CTkFrame(form, fg_color="transparent")
        btn_row.pack(fill="x", pady=(10, 10))
        ctk.CTkButton(btn_row, text=self._t("add_maint"), command=self.on_add_maintenance).pack(anchor="w")
        ...
        def _update_room_filter(*_):
            ...
            filtered = [f"{r.room_id} — {r.display_name}" for r in self.system.rooms.values() if r.floor == fl]
        ...
        act = ctk.CTkFrame(fr, fg_color="transparent")
        act.pack(fill="x", pady=(12, 0))
        ctk.CTkButton(act, text=self._t("remove_maint"), fg_color=self.colors["danger"], hover_color=self.colors["danger"], command=self.on_remove_maintenance).pack(side="left")
        return fr


    def _page_reports(self, master) -> ctk.CTkFrame:
        fr = ctk.CTkFrame(master, fg_color="transparent")
        card = self._card(fr, self._t("reports_title"), self._t("reports_subtitle"))
        card.pack(fill="both", expand=True)
        body = card.body  # type: ignore

        ctk.CTkButton(body, text=self._t("export_occ"), command=self.on_export_occupancy).pack(anchor="w", pady=6)
        ctk.CTkButton(body, text=self._t("export_rev"), command=self.on_export_revenue).pack(anchor="w", pady=6)

        ctk.CTkFrame(body, fg_color=self.colors["line"], height=2).pack(fill="x", pady=15)

        self.summary_var = ctk.StringVar(value="—")
        ctk.CTkLabel(body, text=self._t("quick_summary"), font=("Segoe UI", 16, "bold")).pack(anchor="w", pady=(0, 6))
    ...
    def _page_help(self, master) -> ctk.CTkFrame:
        ...
        ctk.CTkLabel(head, text=self._t("help_title"), font=("Segoe UI", 20, "bold")).pack(side="left")
        ctk.CTkLabel(head, text=self._t("help_subtitle"), text_color=self.colors["muted"], 
        ...
        # 1. AVL Tree
        algo_card(scroll, "Dengeli Arama Ağacı (AVL)", "Veri Yapısı",
            [
                "Kullanım: Oda indeksleme (room_id -> Room nesnesi).",
                "Neden: Python dict yerine, dengeli ağaç yapısının performansını göstermek için.",
                "Mekanizma: Her eklemede (insert) yükseklik farkı (balance factor) kontrol edilir.",
                "Rotasyon: Dengesizlik varsa LL, RR, LR, RL rotasyonları ile ağaç dengelenir.",
                "Self-Balancing: Ağaç her zaman ~log(N) derinliktedir."
            ],
        ...
        algo_card(scroll, "Dinamik Programlama (Knapsack)", "Algoritma",
            [
                "Kullanım: 'Bütçe Sihirbazı' (0/1 Knapsack Problemi).",
                "Senaryo: Bütçenize (W) en uygun tatil paketini (oda + hizmetler) oluşturur.",
                "Formül: dp[i][w] = max(dp[i-1][w], price + dp[i-1][w-price]).",
                "Mantık: Odayı seçtikten sonra, kalan bütçeye en çok 'değer' katan hizmetleri ekler.",
                "Sonuç: Paranızın tam karşılığını veren optimum paket."
            ],
            {"Karmaşıklık": "O(N * W)", "Tür": "Pseudo-Polynomial"}
        )

        # 4. Sorting & Binary Search
        algo_card(scroll, "Sıralama ve Arama", "Algoritma",
            [
                "Merge Sort: Rezervasyon listelerini tarihe göre sıralar (Stable Sort).",
                "Heap Sort: 'Uygun Odalar' listesinde en ucuz 10 odayı bulmak için (Min-Heap).",
                "Binary Search (Lower Bound): Yeni rezervasyonun, mevcut rezervasyonlar arasına",
                "çakışma olmadan eklenebileceği konumu bulmak için kullanılır."
            ],
            {"Merge Sort": "O(N log N)", "Heap Sort": "O(N log N)", "Binary Search": "O(log N)"}
        )

        # 5. Stack & Queue
        algo_card(scroll, "Stack & Queue", "Veri Yapısı",
            [
                "Stack (Yığın): 'Geri Al / İleri Al' (Undo/Redo) sistemi.",
                "  - Her işlem (Command) bir stack'e atılır. Geri alınınca pop edilir.",
                "Queue (Kuyruk): Bekleme Listesi (Waitlist).",
                "  - Oda boşalınca, sıradaki ilk kişi (FIFO) otomatik atanır."
            ],
            {"Erişim": "O(1) (Uçlardan)", "Yapı": "LIFO (Stack) / FIFO (Queue)"}
        )
...
# ---------- Theme (Dark/Light) ----------
    ...
    def refresh_price_preview(self) -> None:
        try:
            ...
            if not avail:
                self.price_preview.set(self._t("no_room_msg"))
            else:
                room = avail[0]
                price = self.system.compute_price(room, ci, co, adults, children, services)
                
                currency = self._t("val_currency")
                nights_lbl = self._t("nights")
                
                # Format localized strings
                l1 = self._t("preview_line1").format(start=fmt_date(ci), end=fmt_date(co), nights=nights)
                l2 = self._t("preview_line2").format(price=price, currency=currency)
                l3 = self._t("preview_line3").format(tm=tm)
                l4 = self._t("preview_line4").format(dm=dm)
                l5 = self._t("preview_line5").format(adults=adults, children=children)
                
                self.price_preview.set(f"{l1}\n{l2}\n\n{l3}\n{l4}\n{l5}")
    ...
    def _draw_availability_bars(self, rtype: RoomType) -> None:
        ...
        # title
        t = self._t("chart_title").format(rtype=rtype)
        c.create_text(pad, pad, anchor="nw", text=t, fill=self.colors["muted"], font=("Segoe UI", 9, "bold"))
    ...
    def on_book(self) -> None:
        try:
            ...
            if not raw_name:
                messagebox.showwarning("Eksik Bilgi", "Lütfen 'Misafir Adı' alanını doldurunuz.")
    ...
    def on_cancel_selected(self) -> None:
        ...
        if not sel:
            messagebox.showinfo("Seçim", "Lütfen bir rezervasyon seçin.")
            return
        item = self.res_tree.item(sel[0])
        res_id = item["values"][0]
        ok, msg = self.system.cancel(res_id)
        if ok:
            messagebox.showinfo("İptal", msg)
        else:
            messagebox.showwarning("İptal", msg)
        self.refresh_all()
    ...
    def on_add_maintenance(self) -> None:
        try:
            ...
            if scope == "room":
                ...
                room_id = choice.split(" — ", 1)[0].strip() if choice else ""
    ...
    def on_remove_maintenance(self) -> None:
        ...
        if not sel:
            messagebox.showinfo("Seçim", "Lütfen bir tadilat kaydı seçin.")
            return
    ...
    def on_export_occupancy(self) -> None:
        ...
        messagebox.showinfo("CSV", f"Yazıldı: {path}")
    ...
    # ---------------------------------------------------------
    # EKLE: ModernHotelApp Class'ının içine (Actions bölümü)
    # ---------------------------------------------------------
    def on_knapsack_suggest(self) -> None:
        ...
        # Bütçeye uygun FULL paketleri (Oda + Hizmet) ara
        packages = self.system.suggest_holiday_packages(budget, checkin_date, nights, adults, children)
        
        if not packages:
            messagebox.showinfo("Sonuç", "Bu bütçe ve kriterlere uygun tatil paketi bulunamadı.\nLütfen bütçeyi artırmayı veya tarih değiştirmeyi deneyin.")
            return
            
        # Sonuçları göster ve seçtir
        # Callback için lambda kullanırken adults/children'ı da paketliyoruz (ResultDialog'da zaten paket var ama)
        # Packages listesine adults/children ekleyelim ki apply ederken kullanalım
        for p in packages:
             p['adults'] = adults
             p['children'] = children
             p['nights'] = nights
    ...
    def apply_reservation_package(self, pkg: dict):
        """
        Seçilen paketi Rezervasyon ekranına uygular:
        1. Tab'ı değiştir
        2. Tarih, Kişi, Gece doldur
        3. Hizmetleri seç
        4. Odayı bul ve listede seçili hale getir
        """
        room = pkg["room"]
        checkin = pkg["checkin"]
        nights = pkg["nights"]
        ...
        # Oda tipini, seçilen odanın tipine ayarla (filtreleme için)
        if hasattr(self, "type_var"):
        ...
        # 5. Listede odayı bul ve seç
        # Treeview'da her satırın values=(RoomDisplayName, ..., ..., ..., Price)
        # Ancak biz arka planda populate yaparken iid olarak room_id kullanıyor olabiliriz veya values[0]
        # _populate_rooms metoduna bakmak lazım ama şimdilik oda adından veya tipinden bulmaya çalışalım.
        # En sağlıklısı room.room_id.
        
        # Treeview'da arayalım
        found = None
        for item_id in self.av_tree.get_children():
            # item_id genellikle bizim insert şeklimize göre değişir.
            # Kodda: self.av_tree.insert("", "end", iid=room.room_id, values=...)
            # Eğer iid olarak room_id kullanıldıysa:
        ...
        if found:
            ...
            messagebox.showinfo("Paket Seçildi", f"'{room.display_name}' odası ve hizmetler seçildi.\nLütfen 'Misafir Adı' girip Rezervasyon Yap'a basın.")
            
            # 6. İsme odaklan (Eğer referansı varsa)
            if hasattr(self, "entry_guest"):
                self.entry_guest.focus_set()
        else:
            messagebox.showwarning("Dikkat", "Paket uygulandı ancak oda listede bulunamadı.\n(Belki başkası kaptı veya filtre sorunu).")
    ...
    def refresh_reservations(self) -> None:
        ...
        mode = self._t("active") # default to translated Active
        if hasattr(self, "reservations_view_mode"):
            mode = self.reservations_view_mode.get()
        ...
        if hasattr(self, "res_tree") and self.res_tree.winfo_exists():
            ...
            for r in rows:
                # Filter by Mode (Active vs Archive)
                r_checkout = parse_date(r.check_out)
                is_past = (r_checkout < today_date)
                
                # Check against translated strings
                if mode == self._t("active") and is_past:
                    continue
                if mode == self._t("archive") and not is_past:
                    continue
    ...
    def refresh_summary(self) -> None:
        ...
        if hasattr(self, "summary_var"):
            self.summary_var.set(
                ...
                f"Önümüzdeki 30 gün (girişe göre) gelir: {rev_30} TL\n"
        ...
        if hasattr(self, "kpi_var"):
            self.kpi_var.set(f"Rezervasyon: {res_count}  •  30g gelir: {rev_30} TL")
